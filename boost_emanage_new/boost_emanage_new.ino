...
const int Forsunka =  17;        // номер выхода, подключенного к форсунке
const int BoostPin =  11;        // номер выхода, подключенного к соленоиду
const int DiffPin =  12;
const int OrosheniePin =  14;        // номер выхода, подключенного к орошению
...
unsigned long pMDutyDiff=0;               // Время питания муфты
...
int FrencSolDiff = 150;
int Odlit = 0;
int Oprom = 0;
boolean AutoDiffMode = false;
int DiffModeForSave = 101;
...
void setup() {
 ...
 // pinMode(OrosheniePin, OUTPUT);
  ...
  PerTimeDiff = 1000000/FrencSolDiff;
  ...
  SetDutyMin = EEP[3];
  OroshenieOrd = EEP[4];
  DiffModeForSave = EEP[5];
  
  SetBaseDuty(MaxBoostConf);
  
  if (DiffModeForSave == 101) AutoDiffMode = true; else DutyWDiff = DiffModeForSave;
...
void loop() {
    ...
    if (micros()-pMDutyDiff >= PerTimeDiff){
      if (DutyWDiff > 0) digitalWrite(DiffPin, HIGH); // Питание на муфту
      pMDutyDiff = micros(); // 
    }
    ...
    if (AutoDiffMode) DutyWDiff = map(Boost ,0 ,100 , 10, 80); // Автоматический режимм центрального диффа
            
    if (DutyW > 100) DutyW=100;
    if (DutyW < 0) DutyW=0;
    if (DutyWDiff > 100) DutyWDiff=100;
    if (DutyWDiff < 0) DutyWDiff=0;
 ...
 //   }  
  ...
  if(micros()-pMDutyDiff >= DutyWDiff*PerTimeDiff/100) // Если прошел период открытия, закрываем соленоид
  {
  ...
  if(millis() - previousMillisButton > interval_Button) {
    ...
    if (Button1Time >= 50 && Button1Time < 1000) // Отпустили кнопку настроек (короткое нажатие)
    {
      ...
      if(!ConfMode)
      {
        ...
        DiffModeForSave = DutyWDiff;
        EEPROM.write(5, DiffModeForSave);
      }
    }
    
    if (Button2Time > 1000) // Отпустили кнопку настроек (длинное нажатие)
    {
      AutoDiffMode = !AutoDiffMode; 
      if (AutoDiffMode) DiffModeForSave=101; else DiffModeForSave = DutyWDiff;
      EEPROM.write(5, DiffModeForSave); 
    }
    if (Button2Time >= 50 && Button2Time < 1000) // ************************************** Отпустили кнопку 2
    {
